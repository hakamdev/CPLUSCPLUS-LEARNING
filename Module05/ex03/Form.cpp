...
/*   Updated: 2022/02/20 23:42:26 by ehakam           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Form::beSigned(Bureaucrat const & b) {
	if (this->_isSigned) {
		std::cout << this->_name << " Form is already been signed!" << std::endl;
		return ;
	}
...
Form::GradeTooLowException::GradeTooLowException( std::string name )
	: _thower_name(name) {}
Form::FormNotSignedException::FormNotSignedException( std::string name )
	: _thower_name(name) {}
...
const char*	Form::FormNotSignedException::what () const throw () {
	std::string message = std::string("FormNotSignedException:\n")  + _thower_name + ": is not signed!";
	return (message.c_str());
}
